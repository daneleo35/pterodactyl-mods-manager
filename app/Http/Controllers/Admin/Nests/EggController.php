...
use Pterodactyl\Services\ServerMods\EggModSupportService;
...
class EggController extends Controller
{
    ...
    public function __construct(
        ...
        protected EggModSupportService $eggModSupportService,
    ...
    public function view(Egg $egg): View
    {
        return view('admin.eggs.view', [
            ...
            'modsOverride' => $this->eggModSupportService->getOverride($egg),
            'modsGameOptions' => $this->eggModSupportService->gameOptions(),
            'modsSourceOptions' => $this->eggModSupportService->sourceOptions(),
            'modsSearchProviderOptions' => $this->eggModSupportService->searchProviderOptions(),
        ]);
    }
    ...
    {
        $data = $request->validated();
        $overrideData = Arr::only($data, [
            'mods_override_enabled',
            'mods_override_game',
            'mods_override_catalog_search_provider',
            'mods_override_supported_kinds',
            'mods_override_sources',
        ]);
        $data = Arr::except($data, array_keys($overrideData));
        $data['docker_images'] = $this->normalizeDockerImages($data['docker_images'] ?? null);

        $this->updateService->handle($egg, $data);
        $this->eggModSupportService->saveOverride($egg, $overrideData);
