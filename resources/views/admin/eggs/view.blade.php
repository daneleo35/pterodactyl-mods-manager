            ...
            <div class="box">
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="col-xs-12">
                            <hr>
                            <h4>Mod Support Override</h4>
                            <p class="text-muted small">Use this when automatic mod detection misses a game, source provider, or built-in search provider for this egg.</p>
                        </div>
                        <div class="col-sm-4">
                            <div class="checkbox checkbox-primary">
                                <input id="pModsOverrideEnabled" name="mods_override_enabled" type="checkbox" value="1" @if($modsOverride['enabled']) checked @endif />
                                <label for="pModsOverrideEnabled" class="strong">Enable Override</label>
                            </div>
                            <p class="text-muted small">Forces the Mods tab to use the values below for servers on this egg.</p>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label for="pModsOverrideGame" class="control-label">Game</label>
                                <select id="pModsOverrideGame" name="mods_override_game" class="form-control">
                                    @foreach($modsGameOptions as $value => $label)
                                        <option value="{{ $value }}" @if(($modsOverride['game'] ?? '') === $value) selected @endif>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label for="pModsOverrideCatalogSearchProvider" class="control-label">Built-in Search Provider</label>
                                <select id="pModsOverrideCatalogSearchProvider" name="mods_override_catalog_search_provider" class="form-control">
                                    @foreach($modsSearchProviderOptions as $value => $label)
                                        <option value="{{ $value }}" @if(($modsOverride['catalog_search_provider'] ?? '') === $value) selected @endif>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="pModsOverrideKinds" class="control-label">Supported Content Types</label>
                                <select id="pModsOverrideKinds" name="mods_override_supported_kinds[]" class="form-control" multiple>
                                    @foreach(['mod' => 'Mods', 'plugin' => 'Plugins', 'datapack' => 'Datapacks'] as $value => $label)
                                        <option value="{{ $value }}" @if(in_array($value, $modsOverride['supported_kinds'] ?? [])) selected @endif>{{ $label }}</option>
                                    @endforeach
                                </select>
                                <p class="text-muted small">Choose what kind of content should show in the Mods tab.</p>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for="pModsOverrideSources" class="control-label">Source Providers</label>
                                <select id="pModsOverrideSources" name="mods_override_sources[]" class="form-control" multiple>
                                    @foreach($modsSourceOptions as $value => $label)
                                        <option value="{{ $value }}" @if(in_array($value, $modsOverride['sources'] ?? [])) selected @endif>{{ $label }}</option>
                                    @endforeach
                                </select>
                                <p class="text-muted small">Examples: Nexus for Windrose, CurseForge for Hytale, or Steam Workshop for Workshop-driven games.</p>
                            </div>
                        </div>
                    </div>
                </div>
...
@section('footer-scripts')
    ...
    $('#pModsOverrideKinds').select2();
    $('#pModsOverrideSources').select2();
    </script>
@endsection
